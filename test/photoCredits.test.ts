...
describe('PhotoCredits', () => {
    it('resolves on normal input name', () => {
        ...
        expect(plainGetPhotoCredits('http://localhost:4444/wp-content/uploads/2022/07/Helen-Woigk%C2%A9Christian-van-dalen-opt.jpg')).toBe('Christian Van Dalen')
        ...
        expect(plainGetPhotoCredits('http://localhost:4444/wp-content/uploads/2022/05/Franz-Xaver-Zeller©-Elena-Zaucke.jpg')).toBe('Elena Zaucke')
    })

    it('resolves on somewhat normal input name', () => {
        expect(plainGetPhotoCredits('Christian A. Koch_© Jan Merkle (29).jpg')).toBe('Jan Merkle')
        expect(plainGetPhotoCredits('Christian A. Koch_©_Jan_Merkle_(29).jpg')).toBe('Jan Merkle')
