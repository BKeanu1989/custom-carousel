...
export function horizontalLoop(
  items,
  config,
  { aniEnd: cb, onComplete: cb2, onCompleteSecond: onCompleteFn, onReset: cb3 }
) {
  ...
  // console.log("🚀 ~ file: gsapUtils.ts ~ line 39 ~ horizontalLoop ~ totalWidth", totalWidth)
  for (i = 0; i < length; i++) {
    item = items[i];
    ...
    // TODO: backup for xPercent: snap((curX - distanceToLoop) / widths[i] * 100)
    tl.to(
      item,
      {
...
export function horizontalLoop3(items, config) {
  items = gsap.utils.toArray(items);
  config = config || {};
  let onChange = config.onChange,
    lastIndex = 0,
